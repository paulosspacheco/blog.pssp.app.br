...
export class InputBox {
    ...
    adjustDialogWidth(dialog) {
        ...
        Array.from(dialog.querySelectorAll('.form-row')).forEach(row => {
            let rowWidth = 0;
...
// Função para ajustar a altura da caixa de diálogo
    adjustDialogHeight(dialog) {
        ...
        const totalHeight = Array.from(dialog.querySelectorAll('.form-row')).reduce((sum, row) => {
    ...
    showInputBox() {
        ...
        formFieldsContainer.className = 'form-fields'; // Use a nova classe para os campos
    
        // Adiciona campos
        this.fields.forEach((line, lineIndex) => {
            const fieldsetElement = document.createElement('div');
            fieldsetElement.className = 'form-row';

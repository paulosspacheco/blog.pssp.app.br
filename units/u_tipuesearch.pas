...
procedure TIndex_Tipuesearch.tipuesearch_Undefined(Sender: TObject;
  ...
  begin
    try
      v := query(
      'for $x in doc("file:///'+ExpandFileName(aFileName)+'")/html/body'+
//      ' order by $x'+
      ' return <li>{data($x)}</li>'
      );
      result := v.toString;
      ...
      // remove todas a palavras menos x caracteres
//      result := filterSizeChar(result);
      ...
      //Result := ChangeSubStr(CR  , '/' ,Result);
      //Result := ChangeSubStr(LF  , '/' ,Result);
      //Result := ChangeSubStr('➚' , '/' ,Result);
      //Result := ChangeSubStr('🔝' , '/' ,Result);
      //Result := ChangeSubStr('🔙' , '/' ,Result);
